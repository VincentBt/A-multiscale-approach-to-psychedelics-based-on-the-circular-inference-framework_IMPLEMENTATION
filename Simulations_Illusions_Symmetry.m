...
%%% Illusions - P=1 (Positive prior)
%%% Symmetry

%% PARAMETERS
Nrep = 30; % Nb of repetitions of BP
ws1 = 0.95; % FF weight
ws0 = 0.05;
wp1 = 0.95; % FB weight
wp0 = 0.05;
La = -2:0.5:1; % auditory input
Lv = -2:0.5:1; % visual input
P = 1; % prior
...
BA6=zeros(Nrep,length(La),length(Lv));
BA7=zeros(Nrep,length(La),length(Lv));
...
MA7A6=zeros(Nrep,length(La),length(Lv)); % Messages
MA6A7=zeros(Nrep,length(La),length(Lv));
MA6A5=zeros(Nrep,length(La),length(Lv));
MA5A6=zeros(Nrep,length(La),length(Lv));
...
for a = 1:length(La) % BP
    for v = 1:length(Lv)
        for rep = 1:Nrep-1
            BB(rep+1,a,v) = MAB(rep,a,v) + MVB(rep,a,v) + P;
            ...
            BA5(rep+1,a,v) = MA4A5(rep,a,v) + MA6A5(rep,a,v);
            BA6(rep+1,a,v) = MA5A6(rep,a,v) + MA7A6(rep,a,v);
            BA7(rep+1,a,v) = MA6A7(rep,a,v) + La(a);
            ...
            MA6A5(rep+1,a,v) = F(BA6(rep+1,a,v) - (ad * MA5A6(rep,a,v)),ws1,ws0);
            MA5A6(rep+1,a,v) = F(BA5(rep+1,a,v) - (ac * MA6A5(rep,a,v)),wp1,wp0);
            MA7A6(rep+1,a,v) = F(BA7(rep+1,a,v) - (ad * MA6A7(rep,a,v)),ws1,ws0);
            MA6A7(rep+1,a,v) = F(BA6(rep+1,a,v) - (ac * MA7A6(rep,a,v)),wp1,wp0);
...
bA6i = exp(BA6)./(1+exp(BA6));
bV7i = exp(BV7)./(1+exp(BV7));
bA7i = exp(BA7)./(1+exp(BA7));
...
BV7=zeros(Nrep,length(La),length(Lv));
BB = zeros(Nrep,length(La),length(Lv));
MA7A6=zeros(Nrep,length(La),length(Lv));
MA6A7=zeros(Nrep,length(La),length(Lv));
MA6A5=zeros(Nrep,length(La),length(Lv));
MA5A6=zeros(Nrep,length(La),length(Lv));
MA5A4=zeros(Nrep,length(La),length(Lv));
...
bA6d = exp(BA6)./(1+exp(BA6));
bV7d = exp(BV7)./(1+exp(BV7));
bA7d = exp(BA7)./(1+exp(BA7));
...
bA6c = exp(BA6)./(1+exp(BA6));
bV7c = exp(BV7)./(1+exp(BV7));
bA7c = exp(BA7)./(1+exp(BA7));
...
figure(1) % No Loops
subplot(1,2,1)
surf(Lv,La,squeeze(bV4i(end,:,:)))
view(2)
xlabel('Lv')
ylabel('La')
title('Bv')
subplot(1,2,2)
surf(Lv,La,squeeze(bA4i(end,:,:)))
view(2)
xlabel('Lv')
ylabel('La')
title('Ba')
...
subplot(1,2,1)
surf(Lv,La,squeeze(bV4d(end,:,:)))
view(2)
xlabel('Lv')
ylabel('La')
title('Bv')
subplot(1,2,2)
surf(Lv,La,squeeze(bA4d(end,:,:)))
view(2)
xlabel('Lv')
ylabel('La')
title('Ba')
...
subplot(1,2,1)
surf(Lv,La,squeeze(bV4c(end,:,:)))
view(2)
xlabel('Lv')
ylabel('La')
title('Bv')
subplot(1,2,2)
surf(Lv,La,squeeze(bA4c(end,:,:)))
view(2)
xlabel('Lv')
ylabel('La')
title('Ba')
